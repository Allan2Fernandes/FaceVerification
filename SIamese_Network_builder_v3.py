...
class Siamese_Network_builder_v3:
    ...
    def get_encoding_layer(self, input_layer, num_of_filters):
        conv_layer0 = Conv2D(padding = 'same', kernel_size=(3,3), strides = (1,1), filters=num_of_filters)(input_layer)
        max_pooling0 = MaxPool2D(padding = 'same', strides = (2,2), pool_size=(2,2))(conv_layer0)

        conv_layer1 = Conv2D(padding = 'same', kernel_size=(3,3), strides = (1,1), filters = num_of_filters*2)(max_pooling0)
        max_pooling1 = MaxPool2D(padding = 'same', strides = (2,2), pool_size=(2,2))(conv_layer1)

        conv_layer2 = Conv2D(padding = 'same', kernel_size=(3,3), strides=(1,1), filters = num_of_filters*4)(max_pooling1)
        max_pooling2 = MaxPool2D(padding='same', strides=(2, 2), pool_size=(2, 2))(conv_layer2)

        conv_layer3 = Conv2D(padding='same', kernel_size=(3,3), strides=(1,1), filters = num_of_filters*8)(max_pooling2)
        conv_layer4 = Conv2D(padding = 'same', kernel_size=(3,3), strides=(1,1), filters = num_of_filters*8)(conv_layer3)
        max_pooling3 = MaxPool2D(padding='same', strides=(2, 2), pool_size=(2, 2))(conv_layer4)

        conv_layer5 = Conv2D(padding='same', kernel_size=(3, 3), strides=(1, 1), filters=num_of_filters*8)(max_pooling3)
        conv_layer6 = Conv2D(padding='same', kernel_size=(3, 3), strides=(1, 1), filters=num_of_filters*8)(conv_layer5)
        max_pooling4 = MaxPool2D(padding='same', strides=(2, 2), pool_size=(2, 2))(conv_layer6)

        flatten_layer = Flatten()(max_pooling4)
        dense_layer0 = Dense(units=512)(flatten_layer)
        dense_layer1 = Dense(units=512)(dense_layer0)

        encoding_layer = Dense(256)(dense_layer1)
